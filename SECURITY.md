...
TimeAssertions / TimeAssertions.TUnit is a personal-name open-source project distributed under the [MIT license](LICENSE). Although the package is intended for use **only in test projects** (see the scope blockquote on every README), security reports are still welcome and treated as a priority.
...
|---|---|---|
| `0.2.x` (TimeAssertions.TUnit) | **Current** | ✅ Yes |
...
- The shipped `TimeAssertions` and `TimeAssertions.TUnit` NuGet packages
...
- Realistic in-product attack surface examples:
  - Unbounded memory or CPU consumption from a crafted assertion chain (e.g. budget renderings)
...
Replace `<package>` (one of `TimeAssertions` or `TimeAssertions.TUnit`) and `<version>` (e.g. `0.2.0`) with the artifact you downloaded:

```bash
# Verify SLSA v1.0 build provenance: was this nupkg built from this audited source tree?
gh attestation verify ./<package>.<version>.nupkg --repo JohnVerheij/TimeAssertions.TUnit
